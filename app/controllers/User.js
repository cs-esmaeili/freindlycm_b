...
const { mAddUser, mDeleteUser, mUpdateUser } = require('../messages/response.json');
...
exports.updateUser = async (req, res) => {
    try {
        const { user_id, name, leader } = req.body;
        const result = await User.updateOne({ _id: user_id }, { name, leader });
        if (result.modifiedCount == 1) {
            res.send({ message: mUpdateUser.ok });
            return;
        }
        throw { message: mUpdateUser.fail, statusCode: 401 };
    } catch (err) {
        res.status(err.statusCode || 422).json(err);
    }
}
